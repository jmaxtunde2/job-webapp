				...
				<h2 class="title1" style="margin-left:30px;">Gestion des Utilisateurs </h2>
					<div class="form-grids row widget-shadow" style="margin-left:30px;margin-right:20px;" data-example-id="basic-forms"> 
						<div class="form-title">
							...
							<a href="{{route(''.$data["user_type"].'.users_index')}}" class="btn btn-success pull-right " style="margin-left:7px;background-color: #F2B33F;"><i class="fa fa-list"> Liste des Utilisateurs</i></a>
                        </div>
						</div>

						 <form  method="POST"  action="{{route(''.$data["user_type"].'.users_index')}}">
							@csrf
						  <div class="form-group col-md-6">
										<label for="field">Saississez le nom ou l'email ou le phone </label>
									<input required  name="field" type="text" class="form-control " id="field" placeholder="Saississez le nom ou l'email ou le phone">
